...
#note: this is used for portage to properly emerge some package in muslx32.  it will be moved in the portage package and removed from the muslx32toolkit soon.
...
#Maintainer notes:
#The following apply patches for the muslx32 build system.  It is easier to apply patches at this point than to patch each ebuild and then force rebuilding any Makefile*.  The problem can in coreutil's test shell command, automake, libtool or files generated from these 
#tools distributed with the package.

#echo "$EBUILD_PHASE"
if [[ $EBUILD_PHASE == configure ]]; then

	#Maintainer notes:
	#The following commented block are just backup for the below uncommented patches.
	...
	#Maintainer notes:
	#The following are just backup for the above uncommented patches.
